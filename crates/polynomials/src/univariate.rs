...
impl<P: FieldParams, const N: usize> Univariate<P, N> {
    ...
    /// Serialize evaluations to a byte buffer (N × 32 bytes, no length prefix).
    pub fn to_buffer(&self) -> Vec<u8> {
        let mut buf = Vec::with_capacity(N * 32);
        for eval in &self.evaluations {
            buf.extend_from_slice(&eval.to_be_bytes());
        }
        buf
    }

    /// Deserialize evaluations from a byte buffer (N × 32 bytes).
    pub fn serialize_from_buffer(buffer: &[u8]) -> Self {
        let mut evaluations = [Field::zero(); N];
        for i in 0..N {
            let bytes: [u8; 32] = buffer[i * 32..(i + 1) * 32].try_into().unwrap();
            evaluations[i] = Field::from_be_bytes(&bytes);
        }
        Self { evaluations }
    }
