...
const NUM_ROUNDS: usize = 64; // 8 full + 56 partial
const T: usize = 4;

/// Parse a hex string (without 0x prefix) into an Fr element via from_limbs.
fn hex_to_fr(hex: &str) -> Fr {
    let hex = hex.strip_prefix("0x").unwrap_or(hex);
    // Pad to 64 hex chars (256 bits)
    assert!(hex.len() <= 64, "hex string too long");
    let padded = format!("{:0>64}", hex);
    // Split into 4 x 16-char chunks (big-endian), convert to u64 limbs (little-endian)
    ...
    Fr::from_limbs([limb0, limb1, limb2, limb3])
}

/// Hex strings for the 4-element internal matrix diagonal.
/// Source: poseidon2_params.hpp lines 24-29
const DIAG_HEX: [&str; T] = [
...
/// Hex strings for round constants: 64 rounds x 4 elements.
/// Rounds 0-3: full external (all 4 nonzero)
/// Rounds 4-59: partial internal (only element 0 nonzero)
/// Rounds 60-63: full external (all 4 nonzero)
/// Source: poseidon2_params.hpp lines 58-443
const RC_HEX: [[&str; T]; NUM_ROUNDS] = [
    // Round 0 (full external)
    [
...
/// Round constants: 64 rounds x 4 Fr elements.
/// Lazily converted from hex strings to Montgomery-form field elements.
pub static ROUND_CONSTANTS: LazyLock<[[Fr; T]; NUM_ROUNDS]> = LazyLock::new(|| {
    let mut rc = [[Fr::zero(); T]; NUM_ROUNDS];
    for i in 0..NUM_ROUNDS {
        for j in 0..T {
            rc[i][j] = hex_to_fr(RC_HEX[i][j]);
        }
    }
    rc
});

/// Internal matrix diagonal (4 elements).
/// The internal MDS matrix is: M_I[i][j] = diagonal[i] if i==j, else 1.
pub static INTERNAL_DIAGONAL: LazyLock<[Fr; T]> = LazyLock::new(|| {
    DIAG_HEX.map(hex_to_fr)
});

/// Test vector input: [0, 1, 2, 3]
pub static TEST_VECTOR_INPUT: LazyLock<[Fr; T]> = LazyLock::new(|| {
    [
        Fr::zero(),
        Fr::one(),
        Fr::from(2u64),
...
/// Test vector output for permutation([0, 1, 2, 3]).
/// Source: poseidon2_params.hpp lines 451-456
pub static TEST_VECTOR_OUTPUT: LazyLock<[Fr; T]> = LazyLock::new(|| {
    [
        hex_to_fr("01bd538c2ee014ed5141b29e9ae240bf8db3fe5b9a38629a9647cf8d76c01737"),
        hex_to_fr("239b62e7db98aa3a2a8f6a0d2fa1709e7a35959aa6c7034814d9daa90cbac662"),
        hex_to_fr("04cbb44c61d928ed06808456bf758cbf0c18d1e15a7b6dbc8245fa7515d5e3cb"),
        hex_to_fr("2e11c5cff2a22c64d01304b778d78f6998eff1ab73163a35603f54794c30847a"),
    ]
});
