...
use crate::polynomials::univariate::Univariate;
...
impl<P: FieldParams> UnivariateCoefficientBasis<P, 2> {
    ...
    pub fn mul(&self, other: &UnivariateCoefficientBasis<P, 2>) -> UnivariateCoefficientBasis<P, 3> {
        ...
        // coefficients[2] already stores a0+a1 for each operand
        result.coefficients[1] =
            self.coefficients[2] * other.coefficients[2] - result.coefficients[0];
        result
    }
    ...
    pub fn sqr(&self) -> UnivariateCoefficientBasis<P, 3> {
        ...
        // result[1] = 2*a0*a1 + a1^2 = (2*a0 + a1) * a1 = (a0+a1 + a0) * a1
        // coefficients[2] = a0+a1, so (coefficients[2] + coefficients[0]) * coefficients[1]
        result.coefficients[1] =
            (self.coefficients[2] + self.coefficients[0]) * self.coefficients[1];
        result
    }
}
